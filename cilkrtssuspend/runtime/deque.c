...
__cilkrts_deque_link* __cilkrts_get_deque_link(void) {
  return &(((deque*)__cilkrts_get_deque())->link);
}
                      ...
                      deque *d, cilk_fiber* fiber)
{
  ...
  if (w->l->batch_id == -1) (*w->l->active_deque)->team = d->team;
...
cilk_fiber* deque_suspend(__cilkrts_worker *w, deque *new_deque)
{
  ...
  if (!new_deque) {
    ...
    if (size > 0) {
      ...
      CilkSteal(kResume, new_deque->SD, new_deque->FOM, w, w);
    ...
    *w->l->active_deque = new_deque;
    ...
    if (attempt_steal && can_steal_from(w,d)) {
        ...
        if (proceed_with_steal) {
            void *SD = CilkSteal(kSuspended, d->SD, d->FOM, w, w);
            (*(w->l->active_deque))->SD = SD;
            #ifdef COLLECT_STEAL_STATS
...
void __cilkrts_set_deque_SD(void *d, void *SD) {
  ((struct deque*)d)->SD = SD;
}

void __cilkrts_set_deque_FOM(void *d, void *FOM) {
  ((struct deque*)d)->FOM = FOM;
}

void __cilkrts_reset_FOM_SD() {
    global_state_t* g = cilkg_get_global_state();
    //fprintf(stderr, "num: %d\n", g->total_workers);
    for (int i = 0; i < g->total_workers; ++i) {
        (*g->workers[i]->l->active_deque)->FOM = NULL;
        (*g->workers[i]->l->active_deque)->SD  = NULL;
    }
}
