...
void __cilkrts_resume_suspended(void* _deque, int enable_resume)
{
  ...
  // If the current stack frame has a parent, our deque is non-empty
  if (/*w->l->type == WORKER_IO ||*/ w->current_stack_frame->call_parent) {
    __cilkrts_make_resumable(_deque);
    return;
  }
  ...
  //CilkSteal(kResume, deque_to_resume->SD, deque_to_resume->FOM, w, NULL);
  /* w->l->mugged--; */
  /* CILK_ASSERT(w->l->mugged >= 0); */
  CILK_ASSERT(deque_to_resume->self == INVALID_DEQUE_INDEX);
  if (mugged) {
