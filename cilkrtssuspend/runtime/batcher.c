...
{
  __cilkrts_worker* w = __cilkrts_get_tls_worker();
  //fprintf(stderr, "terminate_batch %d\n", w->self);
  //assert((w->g->pending_batch.id <= w->l->batch_id) && w->g->batch_lock.owner);
  int p = cilkg_get_nworkers();
  ...
  CILK_ASSERT(!w->l->batch_deque->frame_ff);
...
void call_batch(__cilkrts_worker* w, struct batch* b)
{
  ...
  if (w->l->batch_deque->frame_ff)
    CILK_ASSERT(w->l->batch_deque->frame_ff->call_stack == NULL);
}
...
void invoke_batch(cilk_fiber *fiber)
{
  ...
  CILK_ASSERT(w->l->batch_deque->frame_ff == NULL);

  // 1. Make sure deque (ltq) pointers are correctly set.
  CILK_ASSERT(!w->l->batch_deque->frame_ff);
  CILK_ASSERT(w->tail == &w->l->batch_deque->tail);
  ...
  *w->l->frame_ff = create_batch_frame(w, fiber);
  //fprintf(stderr, "Worker %i invoking batch %i, frame %p batch: %d.\n", w->self, b->id, *w->l->frame_ff, w->l->batch_id);

  call_batch(w, b);

  w = __cilkrts_get_tls_worker();

  CILK_ASSERT(w->l->batch_deque->head == w->l->batch_deque->tail);
...
void execute_batch(__cilkrts_worker* w, cilk_fiber* fiber, int batch_id)
{
  ...
  //fprintf(stderr, "in the loop 0 %d gb %d b %d lock %p\n", w->self, *global_batch_id, batch_id, w->g->batch_lock.owner);
  ...
  while (((*global_batch_id <= batch_id) && w->g->batch_lock.owner)) {
    ...
    //fprintf(stderr, "in the loop 1 %d\n", w->self);
    ff = pop_next_frame(w);
    if (!ff) {
    ...
    //fprintf(stderr, "in the loop 2 %d\n", w->self);
    if (ff) {
      ...
      CILK_ASSERT(w->l->next_frame_ff == NULL && "Should be null here");
      cilk_fiber_suspend_self_and_resume_other(w->l->scheduling_fiber,
                                               other);
      w = __cilkrts_get_tls_worker();
    }
    //fprintf(stderr, "in the loop 3 %d\n", w->self);
    
    //if (*global_batch_id > batch_id) fprintf(stderr, "terminated %d\n", w->self);
    //if (!w->g->batch_lock.owner) fprintf(stderr, "termianted lock %d\n", w->self);
  }
  /*
  fprintf(stderr,
...
void batch_scheduler_function(cilk_fiber *fiber)
{
  while (1) {
    ...
    CILK_ASSERT(w->l->frame_ff == &w->l->batch_deque->frame_ff);
    CILK_ASSERT(w->l->batch_deque->frame_ff == NULL);
...
cilk_fiber* switch_to_batch_deque(__cilkrts_worker* w)
{
  cilk_fiber* fiber;
  BEGIN_WITH_WORKER_LOCK(w) {
    CILK_ASSERT(w->l->batch_deque->tail == w->l->batch_deque->head);
    CILK_ASSERT(w->l->batch_deque->tail == w->l->batch_deque->exc);
    CILK_ASSERT(w->l->batch_deque->tail == w->l->batch_deque->ltq);
    CILK_ASSERT(w->l->batch_deque->protected_tail == w->l->batch_deque->ltq + w->g->ltqsize);
    CILK_ASSERT(w->l->batch_deque->frame_ff == NULL);

    fiber = cilk_fiber_get_current_fiber();//w->l->core_deque->frame_ff->fiber_self;

    w->tail = &w->l->batch_deque->tail;
    w->head = &w->l->batch_deque->head;
    w->exc = &w->l->batch_deque->exc;
    w->ltq_limit = &w->l->batch_deque->ltq_limit;
    w->protected_tail = &w->l->batch_deque->protected_tail;
    w->l->current_ltq = &w->l->batch_deque->ltq;
    w->l->frame_ff = &w->l->batch_deque->frame_ff;
    w->l->active_deque = &w->l->batch_deque;
    w->l->batch_deque->team = w->l->core_deque->team;
  } END_WITH_WORKER_LOCK(w);
...
{
  BEGIN_WITH_WORKER_LOCK(w) {
    w->tail = &w->l->core_deque->tail;
    w->head = &w->l->core_deque->head;
    w->exc = &w->l->core_deque->exc;
    w->ltq_limit = &w->l->core_deque->ltq_limit;
    w->protected_tail = &w->l->core_deque->protected_tail;
    w->l->current_ltq = &w->l->core_deque->ltq;
    w->l->active_deque = &w->l->core_deque;

    CILK_ASSERT(!w->l->batch_deque->frame_ff);
    w->l->batch_deque->head = w->l->batch_deque->tail = w->l->batch_deque->exc = w->l->batch_deque->ltq;

    w->l->frame_ff = &w->l->core_deque->frame_ff;
    w->l->batch_id = -1;
...
cilk_fiber* try_to_start_batch(__cilkrts_worker* w)
{
  CILK_ASSERT(w->head == &w->l->batch_deque->head);
  ...
  //fprintf(stderr, "try to start batch: worker %d %d\n", w->self, is_batch_owner);
  if (is_batch_owner) {
...
void execute_until_op_done(__cilkrts_worker* w, cilk_fiber* current_fiber)
{
  ...
  CILK_ASSERT(w->l->core_deque->frame_ff);
                            ...
                            batch_data_t data, size_t data_size)
{
  ...
  __cilkrts_worker * w = __cilkrts_get_tls_worker();
  //fprintf(stderr, "cilk_batchify worker: %d batch id %d\n", w->self, w->l->batch_id);
  
  CILK_ASSERT(w->l->frame_ff == &w->l->core_deque->frame_ff);
  CILK_ASSERT((*w->l->active_deque)->team != NULL);
...
{
  //fprintf(stderr, "set batch id: batch %d pending %d worker %d\n", w->l->batch_id, w->g->pending_batch.id, w->self);
  w->l->batch_id = w->g->pending_batch.id;
}
