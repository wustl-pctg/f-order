...
extern size_t total_reader;
extern size_t total_writer;
...
int cilkmerge(ELM *low1, ELM *high1, 
    ELM *low2, ELM *high2, ELM *lowdest);

int __attribute__((noinline)) cilkmerge(ELM *low1, ELM *high1, 
    ELM *low2, ELM *high2, ELM *lowdest) {
  ...
  *(lowdest + lowsize + 1) = *split1;
  cilk::future<int> m;
  //cilk_future_create1(&m, cilkmerge, low1, split1 - 1, low2, split2, lowdest);
  reasync_helper(&m, cilkmerge, low1, split1 - 1, low2, split2, lowdest);
...
int cilksort(ELM *low, ELM *tmp, long size);

int __attribute__((noinline)) cilksort(ELM *low, ELM *tmp, long size) {
  ...
  cilk::future<int> sort_futures[3];

  //START_FIRST_FUTURE_SPAWN;
  //  cilksort_helper(&sort_futures[0], A, tmpA, quarter);
  //END_FUTURE_SPAWN;
  //cilk_future_create1(&sort_futures[0], cilksort, A, tmpA, quarter);
  reasync_helper(&sort_futures[0], cilksort, A, tmpA, quarter);
  
  //START_FUTURE_SPAWN;
  //  cilksort_helper(&sort_futures[1], B, tmpB, quarter);
  //END_FUTURE_SPAWN;
  //cilk_future_create1(&sort_futures[1], cilksort, B, tmpB, quarter);
  reasync_helper(&sort_futures[1], cilksort, B, tmpB, quarter);

  //START_FUTURE_SPAWN;
  //  cilksort_helper(&sort_futures[2], C, tmpC, quarter);
  //END_FUTURE_SPAWN;
  //cilk_future_create1(&sort_futures[2], cilksort, C, tmpC, quarter);
  reasync_helper(&sort_futures[2], cilksort, C, tmpC, quarter);
  ...
  //END_FUTURE_SPAWN;
  //cilk_future_create1(&m, cilkmerge, A, A+quarter-1, B, B+quarter-1, tmpA);
  reasync_helper(&m, cilkmerge, A, A+quarter-1, B, B+quarter-1, tmpA);
...
int main(int argc, char **argv) {
  ...
  //printf("total reader: %zu\n", total_reader);
  //printf("total writer: %zu\n", total_writer);
