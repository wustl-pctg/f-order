...
void __attribute__((noinline)) for_loop_helper(const public_struct& pub, private_struct* priv, int start, int end) {
    int count = end - start;
    int mid;

    while (count > 1) {
      mid = start + count/2;
      cilk_spawn for_loop_helper(pub, priv, start, mid);
      start = mid;
      count = end - start;
    }

    compute_kernel(&pub, &(priv[start]));
}
...
//==============================================================================
void noop2() {}
#define FORCE_CILK_FUNC cilk_spawn noop2()
...
//    ensure_serial_execution();
    ...
    for(pub.frame_no=0; pub.frame_no<frames_processed; pub.frame_no++) {
        ...
        //	PROCESSING
        //====================================================================================================
        for_loop_helper(pub, priv, 0, pub.allPoints);
